        ...
        <div className="relative overflow-hidden grid fgrid-col-1 xl:grid-cols-[1fr_600px] gap-8 items-start z-10">
          <div className="space-y-5 ">
            <div className="font-(family-name:--trickOrDead) font-normal space-y-4 [text-shadow:0_0_13px_rgba(0,0,0,0.8)]">
              <p className="text-3xl md:text-5xl leading-10">{selectedBook?.bookSubHeading}</p>
              <h1 className="text-5xl md:text-7xl">{selectedBook?.bookTitle}</h1>
            </div>
            ...
            <div className="mt-8">
              <Link href={selectedBook?.purchaseLink} rel="noreferrer">
                <Button buttonImage={OrangeBackground} altText="get-book" text="Get The Book" />
              </Link>
            </div>
          </div>
          <div className="relative w-full max-w-[600px] mx-auto">
            <Image src={bookImage} alt="Tania Griffith" className="inline-block h-auto w-full " />
          </div>
        </div>
        <Image src={PumpkinSpooky} alt="pumpkin-spooky" className="hidden lg:inline-block absolute bottom-0 left-1/2 -translate-x-1/2 -z-2 max-w-3xs" />
        <Image src={BlurLayer} alt="blur-layer" className="absolute -bottom-0 left-0 right-0" />
      </Herobox>
      <main className=" text-white relative overflow-hidden px-8">
        ...
        <section className="relative flex flex-col justify-center items-center text-center space-y-12 h-150 z-50">
          <div>
            ...
            <p>Goosebumps, Coraline, mysterious wishes gone wrong, and twisty hallway horror.</p>
          </div>

          <div className="flex gap-5 items-center justify-center flex-col lg:flex-row">
            <Link href="/free-story">
              <Button buttonImage={OrangeBackground} altText="read-story" text="READ A FREE STORY" />
            </Link>
            <Link href="/scaremoor">
              <Button buttonImage={YellowBackground} altText="see-more" text="See More Scaremoor Books" />
            </Link>
            <Link href="/club">
              <Button buttonImage={OrangeBackgroundLg} altText="join-scaremoor" text="Join the Scaremoor Society" />
            </Link>
          </div>
        </section>
      </main>
