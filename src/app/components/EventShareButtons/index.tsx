...
import { Event, formatEventDate, formatEventLocation } from "../../constants/Events";

interface EventShareButtonsProps {
  event: Event;
}

const EventShareButtons: React.FC<EventShareButtonsProps> = ({ event }) => {
  ...
  const eventUrl = 'https://www.scaremoor.com/events';
  const eventTitle = `${event.title} - Scaremoor Event`;
  const eventDescription = `Join T.L. Griffith for "${event.title}" - ${formatEventDate(event)} at ${formatEventLocation(event)}. ${event.description.substring(0, 100)}...`;

  const shareLinks = {
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(eventUrl)}`,
    twitter: `https://twitter.com/intent/tweet?url=${encodeURIComponent(eventUrl)}&text=${encodeURIComponent(`${eventTitle} - ${formatEventDate(event)} 📚✨`)}&hashtags=Scaremoor,BookEvent,MiddleGradeHorror`,
    ...
    email: `mailto:?subject=${encodeURIComponent(eventTitle)}&body=${encodeURIComponent(`I thought you might be interested in this Scaremoor event:\n\n${eventTitle}\n${formatEventDate(event)}\n${formatEventLocation(event)}\n\n${event.description}\n\nLearn more: ${eventUrl}`)}`
  };
  ...
  return (
    <div className="flex items-center justify-between pt-3 mt-4 border-t border-gray-700/30">
      <span className="text-xs text-gray-500">Share this event:</span>
