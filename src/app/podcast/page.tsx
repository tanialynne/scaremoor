...
const PodcastPage = () => {
  const episodesByBook = getEpisodesGroupedByBook();
    ...
    <>
      ...
      <main className="text-white">
        ...
        >
          ...
          {Object.entries(episodesByBook).map(([bookTitle, episodes]) => (
            <div key={bookTitle} className="pt-12 relative z-50">
              <h3 className="font-trickordead text-3xl text-center mb-8 text-orange-400">
                {bookTitle}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {episodes.map((episode) => (
                  <PodcastCard
                    key={episode.id}
                    title={`Chapter ${episode.chapterNumber}${episode.chapterTitle ? ` - ${episode.chapterTitle}` : ''}`}
                    description={episode.description}
                    episode={`Chapter ${episode.chapterNumber}`}
          ...
          {/* <div className="text-center pt-12 space-y-6 relative z-50">
            <div className="bg-gradient-to-r from-orange-900/20 to-yellow-900/20 rounded-lg p-8 border border-orange-700/30">
              <h3 className="font-trickordead text-3xl mb-4">
